...
def create_static_table_from_file(filepath):
    ...
    # Fixed CATEGPRY_CODING results in all NaN due to the values in the category column have changed over time in the
    # dataset.
    category = list(set(df['category'].tolist()))
    CATEGORY_CODING = {}
    for code in range(len(category)):
        CATEGORY_CODING[category[code]] = code
    ...
    # shradar-industry
    industry = list(set(df['industry'].tolist()))
    INDUSTRY_CODING = {}
    for code in range(len(industry)):
        INDUSTRY_CODING[industry[code]] = code
